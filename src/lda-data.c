...
corpus* read_data(char* data_filename, int numtopics)
{
    ...
    c->docs[nd].doc_gamma = malloc(sizeof(double)*numtopics);
    c->docs[nd].doc_phi = malloc(sizeof(double)*length);
    c->docs[nd].likelihood = 0.0;
    for (n = 0; n < length; n++)
        c->docs[nd].doc_phi[n] = malloc(sizeof(double)*numtopics);
